...
window.__figb = {
  ...
  richTxt: async (segments, opts = {}) => {
    ...
    for (const seg of segments) {
      ...
      if (seg.hex) {
        t.setRangeFills(start, end, [{ type: 'SOLID', color: __figb.hex(seg.hex) }]);
      }
      if (seg.fill) {
  ...
  verify: () => {
    ...
    if (unnamed.length > 0) issues.push(`NAMING: ${unnamed.length} nodes have default names`);
    const textNodes = page.findAll(n => n.type === 'TEXT');
    const emptyText = textNodes.filter(n => n.characters === '');
    if (emptyText.length > 0) issues.push(`EMPTY_TEXT: ${emptyText.length} text nodes have no content`);
    const tiny = allNodes.filter(n => 'width' in n && n.width < 2 && n.height < 2 && n.visible);
    if (tiny.length > 0) issues.push(`TINY: ${tiny.length} nodes are smaller than 2x2px`);
    return {
